...
for epoch in range(config.epoch):
    ...
    print("epoch: %-5s accuracy: %-10.2f test time: %-10.9f minutes" % (str(epoch + 1), accuracy[epoch], time_list[epoch + 1]))
    total = 0
    correct = 0
print('------------------')
print("Average accuracy: %-7.2f Average test time: %-10.9f minutes" % (np.float(np.mean(accuracy)), np.float(np.mean(time_list))))
